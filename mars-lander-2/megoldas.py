...
while True:
    ...
    # Zónán belül: ellenkormányzás, amíg |hSpeed| > tolerancia, különben függőleges
    if inside_zone:
        zone_hs_tol = 3 if h_above < 300 else 6
        if abs(h_speed) > zone_hs_tol:
            # Dinamikus döntés: nagyobb sebességnél nagyobb szögű ellenkormányzás
            if abs(h_speed) > 35:
                target_rotate = 45 if h_speed > 0 else -45
            elif abs(h_speed) > 20:
                target_rotate = 30 if h_speed > 0 else -30
            elif abs(h_speed) > 10:
                target_rotate = 15 if h_speed > 0 else -15
            else:
                target_rotate = 10 if h_speed > 0 else -10
        else:
            target_rotate = 0
    ...
    # Zónán belül, ha még csökkentjük a vízszintes sebességet: maximális vertikális tolóerő
    if inside_zone and abs(h_speed) > zone_hs_tol:
        desired_power = max(desired_power, 4)
